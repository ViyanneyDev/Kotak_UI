...
@Component({
  selector: 'app-repayment-qc',
  templateUrl: './repayment-qc.component.html',
  styleUrls: ['./repayment-qc.component.scss']
})
export class RepaymentQcComponent implements OnInit {
  ...
  save() {
    ...
    const apiUrl = this._global.baseAPIUrl + "BranchInward/RepaymentQC";
  ...
  onSubmit() {
    ...
    if (this.csvData != null && this.csvData != undefined) {
      ...
      const apiUrl = this._global.baseAPIUrl + 'BranchInward/RepaymentQCBulk';
  ...
  downloadFileErrorLog(data: any) {
    const filename = 'RepaymentQClog_CSV'
  ...
  downloadFile() {
    const filename = 'RepaymentQCFormat_CSV';
