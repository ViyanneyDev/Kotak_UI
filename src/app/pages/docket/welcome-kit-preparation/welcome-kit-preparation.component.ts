...
@Component({
  selector: 'app-welcome-kit-preparation',
  templateUrl: './welcome-kit-preparation.component.html',
  styleUrls: ['./welcome-kit-preparation.component.scss']
})
export class WelcomeKitPreparationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //   save: [false, Validators.requiredTrue], 
    // });
    this.DataUploadForm = this.formBuilder.group({
      ...
      //id: [0],
      CSVData: [""],
      saveredio: ['', Validators.required]
    });
    this.pymentForm = this.formBuilder.group({
      filebarcode: ['', Validators.required],
      remark: ['',[Validators.maxLength(150)]],
      user_Token: localStorage.getItem('User_Token'),
      saveredio: ['', Validators.required],
      CreatedBy: localStorage.getItem('UserID'),
    })
    ...
    if (this.csvData != null && this.csvData != undefined) {
      ...
      this._onlineExamService.postData(this.DataUploadForm.value, apiUrl)
        .subscribe(data => {
          ...
          this.showSuccessmessage("Record Save Succesully");
  ...
  getDisplayNames(csvRecordsArr: any) {
    ...
    this._ColNameList[0] = "filebarcode";
    this._ColNameList[1] = "remark";
    return true;
  }

  GetHeaderNames() {
    this._HeaderList = "filebarcode,remark";
  ...
  downloadFileErrorLog(data: any) {
    const filename = 'WelcomeKitPreprationlog_CSV'
  ...
  downloadFile() {
    const filename = 'WelcomeKitPreparationFormat_CSV';
  ...
  prepareTableData(tableData, headerList) {
    ...
    let tableHeader: any = [
      ...
      { field: 'filebarcode', header: 'FILE BARCODE', index: 1 },
