...
@Component({
  selector: 'app-dispatch-inward',
  templateUrl: './dispatch-inward.component.html',
  styleUrls: ['./dispatch-inward.component.scss']
})
export class DispatchInwardComponent implements OnInit {
  ...
  save() {
    ...
    const apiUrl = this._global.baseAPIUrl + "BranchInward/DispatchInward";
  ...
  onSubmit() {
    ...
    if (this.csvDate != null && this.csvDate != undefined) {
      this.DataUploadForm.patchValue({
       // id: localStorage.getItem('UserID'),
        CSVData: this.csvDate,
      //  User_Token: localStorage.getItem('User_Token'),
  ...
  downloadFileErrorLog(data: any) {
    const filename = 'DispatchInwardLog_CSV'
  ...
  downloadFile() {
    const filename = 'DispatchInwardFormat_CSV';
