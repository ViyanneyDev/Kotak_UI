...
@Component({
  selector: 'app-dispatch-to-vendor',
  templateUrl: './dispatch-to-vendor.component.html',
  styleUrls: ['./dispatch-to-vendor.component.scss']
})
export class DispatchToVendorComponent implements OnInit {
  ...
  save() {
    ...
    const apiUrl = this._global.baseAPIUrl + "BranchInward/DispatchToVendor";
  ...
  onSubmit() {
    ...
    if (this.csvData != null && this.csvData != undefined) {
      ...
      const apiUrl = this._global.baseAPIUrl + 'BranchInward/DispatchToVendorBulk';
  ...
  downloadFileErrorLog(data: any) {
    const filename = 'DispatchToVendorlog_CSV'
  ...
  downloadFile() {
    const filename = 'DispatchToVendorFormat_CSV';
