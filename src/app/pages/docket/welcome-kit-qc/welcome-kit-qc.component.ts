...
@Component({
  selector: 'app-welcome-kit-qc',
  templateUrl: './welcome-kit-qc.component.html',
  styleUrls: ['./welcome-kit-qc.component.scss']
})
export class WelcomeKitQCComponent implements OnInit {
  ...
  save() {
    ...
    this._onlineExamService.postData(this.pymentForm.value, apiUrl)
      .subscribe((data) => {
        if (data == 'Record save successfully' || data=='Record save successfully') {
  ...
  downloadFileErrorLog(data: any) {
    const filename = 'WelcomeKitQclog_CSV'
  ...
  downloadFile() {
    const filename = 'WelcomeKitQCFormat_CSV';
