...
@Component({
  selector: 'app-proposal-qc',
  templateUrl: './proposal-qc.component.html',
  styleUrls: ['./proposal-qc.component.scss']
})
export class ProposalQCComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.csvData != null && this.csvData != undefined) {
      ...
      this.ShowErrormessage("please select file & Checkbox");
  ...
  downloadFileErrorLog(data:any) {
    const filename = 'ProposalQCLog_CSV';
  ...
  downloadFile() {
    const filename = 'ProposalQcFormat_CSV';
    // let csvData = "state,region,appl,apac,contract_no,apac_effective_date,product,location,sub_lcoation,tenure,maturity_date,maln_party_id,party_name,agr_value,eml_start_date,pdc_type";
