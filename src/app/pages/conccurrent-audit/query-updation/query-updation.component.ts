...
@Component({
  selector: 'app-query-updation',
  templateUrl: './query-updation.component.html',
  styleUrls: ['./query-updation.component.scss']
})
export class QueryUpdationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.pymentForm = this.formBuilder.group({
      ...
      remark: [''],
      CheckList:['Select',Validators.required],
      user_Token: localStorage.getItem('User_Token'),
      saveredio: ['', Validators.required],
      entry_by: localStorage.getItem('UserID'),
    })
